...
Beads manager is a local web interface for the Beads database in your current directory. It starts one loopback-only server, opens the task manager in your browser, and stays attached to the terminal until you stop it.
...
This pnpm workspace keeps the public CLI in `apps/beads`. The SDK, server, and React application under `packages/` are private and bundled into the published package.
...
pnpm tsx apps/beads/scripts/test-packed.ts
```
...
Only `@herbcaudill/beads` is public. The other workspace packages have `private: true`, and the tarball contains their bundled runtime code rather than `workspace:*` dependencies.
...
pnpm build
pnpm tsx apps/beads/scripts/test-packed.ts
pnpm --filter @herbcaudill/beads publish --access public --provenance
```
