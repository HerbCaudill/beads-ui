...
describe("views/list navigation", () => {
  test("ArrowDown moves focus to same column in next row", async () => {
    ...
    const first_title = /** @type {HTMLElement} */ (
      mount.querySelector("tbody tr.issue-row:nth-child(1) td:nth-child(3) .editable")
    )
    first_title.focus()
    ...
    const second_title = /** @type {HTMLElement} */ (
      mount.querySelector("tbody tr.issue-row:nth-child(2) td:nth-child(3) .editable")
    )
  ...
  test("ArrowUp moves focus to same column in previous row", async () => {
    ...
    const third_title = /** @type {HTMLElement} */ (
      mount.querySelector("tbody tr.issue-row:nth-child(3) td:nth-child(3) .editable")
    )
    third_title.focus()
  ...
  test("does not intercept inside select controls", async () => {
    ...
    const status_select = /** @type {HTMLSelectElement} */ (
      mount.querySelector("tbody tr.issue-row:nth-child(1) td:nth-child(4) select")
    )
    status_select.focus()
  ...
  test("preserves column when moving from ID button", async () => {
    ...
    const id_btn_row1 = /** @type {HTMLButtonElement} */ (
      mount.querySelector("tbody tr.issue-row:nth-child(1) td:nth-child(1) button")
    )
    id_btn_row1.focus()
    id_btn_row1.dispatchEvent(new KeyboardEvent("keydown", { key: "ArrowDown", bubbles: true }))

    const id_btn_row2 = /** @type {HTMLButtonElement} */ (
      mount.querySelector("tbody tr.issue-row:nth-child(2) td:nth-child(1) button")
    )
