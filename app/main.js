  ...
  if (list_mount && issues_root && epics_root && board_root && detail_mount) {
    ...
    function showFatalFromError(err, context) {
      ...
      if (err && typeof err === "object") {
        const any = /** @type {{ message?: unknown, details?: unknown }} */ (err)
    ...
    async function loadWorkspaces() {
      try {
        ...
        if (result && Array.isArray(result.workspaces)) {
          ...
          const savedWorkspace = window.localStorage.getItem("beads-ui.workspace")
...
if (typeof window !== "undefined" && typeof document !== "undefined") {
  window.addEventListener("DOMContentLoaded", () => {
    ...
    try {
      ...
      const sw = /** @type {HTMLInputElement|null} */ (document.getElementById("theme-switch"))
      if (sw) {
