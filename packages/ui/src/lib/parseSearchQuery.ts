import type { SearchFilterField, SearchTerm } from "./searchQueryTypes"

/** Structured field names recognized by the task-search parser. */
const SEARCH_FILTER_FIELDS = new Set<SearchFilterField>([
  "is",
  "label",
  "parent",
  "priority",
...
/** Parse free text and structured filters from one task-search query. */
export function parseSearchQuery(
  /** Raw task-search query. */
  query: string,
): SearchTerm[] {
  ...
  while (index < query.length) {
    ...
    const field = rawTerm.slice(0, colonIndex).toLowerCase() as SearchFilterField
    if (colonIndex > 0 && SEARCH_FILTER_FIELDS.has(field)) {
