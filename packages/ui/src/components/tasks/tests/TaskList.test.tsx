...
describe("TaskList", () => {
  ...
  describe("store persistence", () => {
    ...
    it("persists collapsed state to store", () => {
      ...
      expect(beadsViewStore.getState().statusCollapsedState.open).toBe(false)
      ...
      expect(beadsViewStore.getState().statusCollapsedState.open).toBe(true)
    })

    it("restores collapsed state from store", () => {
      // Pre-set store with Open collapsed
      beadsViewStore.getState().setStatusCollapsedState({
        open: true,
        deferred: true,
        closed: true,
      })
  ...
  describe("epic grouping within status", () => {
    ...
    it("interleaves epics and standalone tasks by priority", () => {
      ...
      const taskButtons = Array.from(openGroup.querySelectorAll("[role='button']"))
        .map((el) => el.textContent)
        .filter(
          (text) => text && (text.includes("P1") || text.includes("P2") || text.includes("P3")),
        )
