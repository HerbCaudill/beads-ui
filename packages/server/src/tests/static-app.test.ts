...
describe("static application", () => {
  it("serves assets and the application shell from the configured directory", async () => {
    const staticDir = await mkdtemp(join(tmpdir(), "beads-static-"))
    const assetsDir = join(staticDir, "assets")
    await mkdir(assetsDir)
    await writeFile(join(staticDir, "index.html"), "<main>Beads manager</main>")
    ...
    const route = await request(app).get("/issues/bd-1")

    expect(asset.status).toBe(200)
    expect(asset.text).toContain("console.log('beads')")
    expect(route.status).toBe(200)
    expect(route.text).toContain("Beads manager")
  })
})
